...
class MapView(Widget):
    ...
    def get_bbox(self, margin=0):
        ...
        top/right (lat2, lon2).
        """
        x1, y1 = self._scatter.to_local(0 - margin, 0 - margin) #hc
        x2, y2 = self._scatter.to_local((self.width + margin),
                               (self.height + margin))
    ...
    def set_zoom_at(self, zoom, x, y, scale=None):
        ...
        if c1 != c2:
            ...
            scatter.apply_transform(Matrix().translate(
                - scatter.x/scale, - scatter.y/scale, 0
            ), post_multiply=True) #HC
            
            x = self.map_source.get_x(zoom, self.lon) - (self._scatter.to_local(self.center[0],self.center[1])[0] ) #/ scale #HC 
    ...
    def on_angle(self, instance, angle): #HC
    ...
    def get_latlon_at(self, x, y, zoom=None):
        ...
        return Coordinate(
            ...
            lon=self.map_source.get_lon(zoom, x + vx)) #HC
    ...
    def do_update(self, dt): 
        ...
        self.lat = self.map_source.get_lat(zoom,
                (self._scatter.to_local(self.center[0],self.center[1])[1])  - self.delta_y) #HC
    ...
    def _rotate_viewport(self, vx, vy, w, h): #HC

        rotation = self._scatter.rotation

        #** HC Rotation transformation
        if rotation <= 90 :
            dy_down = w*sin(radians(rotation))
            dy_up = h*cos(radians(rotation))
            dx_left = w*cos(radians(rotation))
            dx_right = h*sin(radians(rotation))
            w =int(dx_left + dx_right)
            h =int(dy_down + dy_up)
            vy = int(vy - dy_down) 
            return (vx, vy, w, h)
        ...
        if rotation <= 180:
            rotation -= 90
            dy_down = w*cos(radians(rotation))
            dy_up = h*sin(radians(rotation))
            dx_left = w*sin(radians(rotation))
            dx_right = h*cos(radians(rotation))
            w =int(dx_left + dx_right)
            h =int(dy_down + dy_up)
            vx = int(vx - dx_left)
            vy = int(vy - h)
        ...
        if rotation <= 270:
            rotation -= 180
            dy_down = h*cos(radians(rotation))
            dy_up = w*sin(radians(rotation))
            dx_left = w*cos(radians(rotation))
            dx_right = h*sin(radians(rotation))
            w =int(dx_left + dx_right)
            h =int(dy_down + dy_up)
        ...
        if rotation <= 360:
            ...
            dy_down = w*cos(radians(rotation))
            dy_up = h*sin(radians(rotation))
            dx_left = h*cos(radians(rotation))
            dx_right = w*sin(radians(rotation))
            w =int(dx_left + dx_right)
            h =int(dy_down + dy_up)
            vx = int(vx - dx_left)
            return (vx, vy, w, h)
            ...
            for i in range(arm_size):
                if not self.tile_in_tile_map(x, y) and \
                   ...
                   x >= tile_x_first and x <= tile_x_last:
                    #print "load tile x {}, y {}, armsize {}, zoom {}".format(x, y, arm_size, zoom)
    ...
    def load_tile(self, x, y, size, zoom):
        ...
        #print "load tile x {}, y {}".format (x,y)
